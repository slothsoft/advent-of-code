...
public class ParabolicReflectorDishTest {
    [Test]
    public void Example1_TiltNorth() {
        var example = new ParabolicReflectorDish(File.ReadAllLines(@"14\example.txt"));
        example.TiltNorth();
        
        var expected = new ParabolicReflectorDish(File.ReadAllLines(@"14\example_tilted.txt"));
        
        Assert.AreEqual(expected.Input, example.Input);
    }
    ...
    public void Example1() {
        ...
        Assert.AreEqual(136, example.CalculateLoad());
    }

    [Test]
    public void Puzzle1() {
        var example = new ParabolicReflectorDish(File.ReadAllLines(@"14\input.txt"));
        example.TiltNorth();
