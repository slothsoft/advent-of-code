...
/// <summary>
/// <a href="https://adventofcode.com/2023/day/7">Day 7: Camel Cards</a>: Using the new joker rule, find the rank of every hand in your set. What are the new total winnings?
/// </summary>
public class CamelCardsWithJoker {
    public record Hand(params CardType[] Cards) : IComparable<Hand> {
        ...
        private HandType CalculateType() {
            ...
            var jokerCount = 0;
            do {
                var cardType = sortedCards[startIndex];
                var multiple = FindMultiples(sortedCards, startIndex);
                startIndex += multiple;

                // we handle jokers differently
                if (cardType != CardType.J) {
                    multiples.Add(multiple);
                } else {
                    jokerCount = multiple;

                    // we can stop calculating if there are many jokers
                    if (jokerCount >= 4) {
            ...
            multiples.Sort();
            var highestMultiple = multiples[^1] + jokerCount;
    ...
    public CamelCardsWithJoker(IEnumerable<string> input) {
